...
export const specToNodes = (spec: any) =>
  spec.map((func: any) => ({
    ...
    type: "default",
    label: ("_" + func.func)
      .replaceAll(/_./g, (s) => " " + s.charAt(1).toUpperCase())
      .trim(),
    extra_output: [],
  }));
...
const _objectSpecToStr = (spec: any, depth?: number) => {
  ...
  const fields: string = Object.keys(spec)
    .map((key) => `${key}: ${_specToStr(spec[key], nextDepth)}`)
    .join(", ");
...
const _specToStr = (spec: any, depth?: number): string => {
  ...
  if (typeof spec === "object" && spec.hasOwnProperty("type")) {
    ...
    if (spec.type === "tuple") {
      ...
      return (
        "tuple[" +
        spec.value_type.map((s: any) => _specToStr(s, nextDepth)).join(", ") +
        "]"
      );
    }
    if (spec.type === "union") {
