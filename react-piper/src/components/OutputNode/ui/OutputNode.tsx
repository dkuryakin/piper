    ...
    setNodes(
      Array.from(nodeInternals.values()).map((node) => {
        if (node.id === nodeId) {
          node.data = {
            ...node.data,
            output: [...node.data.output, { name: "", handleId: uuid4() }],
          };
        }
...
const DelOutputButton: FC<DelOutputButtonProps> = ({ nodeId, handleId }) => {
    ...
    setNodes(
      Array.from(nodeInternals.values()).map((node) => {
        if (node.id === nodeId) {
          node.data = {
            ...node.data,
            output: node.data.output.filter(
  ...
  return (
    <button className={"nodrag del-output btn"} onClick={onDelOutput}>
      -
    </button>
  );
};
    ...
    setNodes(
      nodes.map((node: any) => {
        if (node.id === nodeId) {
          node.data = {
            ...node.data,
            output: node.data.output.map(
              ({ name, handleId }: IOutputInput) => ({
